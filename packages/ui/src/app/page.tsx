    ...
    <div onClick={onClick}
      ...
      <div className="flex items-center gap-2 flex-wrap mt-2">
        ...
        {card.estimated_complexity && (
          <Badge text={card.estimated_complexity}
            color={card.estimated_complexity === 'large' ? '#EF4444' : card.estimated_complexity === 'medium' ? '#FFD166' : '#22C55E'} />
        )}
    ...
    <div className="fixed inset-0 bg-black/70 z-50 flex items-start justify-end"
      ...
      <div className="bg-navy-mid w-[480px] h-full overflow-y-auto p-6 flex flex-col gap-4">
        ...
        {card.acceptance_criteria.length > 0 && (
          <div>
            <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Acceptance Criteria</div>
        ...
        {card.annotations && (
          <div className="rounded-lg p-3 border" style={{ borderColor: '#EF444444', background: '#EF444411' }}>
            <div className="text-xs font-semibold text-red-400 mb-2">↩ Bounced Back</div>
            <p className="text-sm text-slate-300 mb-2"><strong>Root cause:</strong> {card.annotations.root_cause}</p>
            {card.annotations.suggested_fix && (
              <p className="text-sm text-slate-300"><strong>Suggested fix:</strong> {card.annotations.suggested_fix}</p>
            )}
            {card.annotations.failed_tests.length > 0 && (
              <div className="mt-2">
                ...
                {card.annotations.failed_tests.map((t, i) => (
                  <div key={i} className="text-xs font-mono text-slate-400">{t}</div>
                ))}
              </div>
            )}
          </div>
        ...
        {card.file_scope.length > 0 && (
          <div>
            <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">File Scope</div>
            <div className="flex flex-wrap gap-1">
              {card.file_scope.map((f, i) => (
                <span key={i} className="text-xs font-mono bg-navy px-2 py-1 rounded text-slate-300">{f}</span>
              ))}
            </div>
          </div>
        )}
        ...
        {/* Move card */}
        {nextCol && (
          <button onClick={() => onMove(nextCol.id)}
        ...
        {/* Agent Log */}
        <div>
          <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Agent Log</div>
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {[...card.agent_log].reverse().map((entry, i) => (
              <div key={i} className="text-xs border-l-2 border-navy pl-3 py-1" style={{ borderColor: '#243B55' }}>
                <div className="flex gap-2 items-center mb-0.5">
                  <span className="font-semibold" style={{ color: '#00C9A7' }}>{entry.agent}</span>
                  <span className="text-slate-400">{entry.action}</span>
                  <span className="text-slate-500 ml-auto">{new Date(entry.timestamp).toLocaleTimeString()}</span>
                </div>
...
export default function BoardPage() {
  ...
  useEffect(() => {
    ...
    if (['card:created', 'card:updated', 'card:column_changed', 'card:log_appended', 'card:bounced'].includes(msg.type)) {
      loadCards();
    ...
    if (msg.type === 'agent:live_feed') {
      ...
      setLiveFeed(prev => [...prev.slice(-49), { message: p.message, ts: new Date().toLocaleTimeString() }]);
    }
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden">
      {/* Top nav */}
      <header className="flex items-center gap-4 px-5 py-3 border-b border-navy-light flex-shrink-0"
      ...
      <div className="flex flex-1 overflow-hidden">
              ...
              <div key={col.id} className="flex flex-col w-52 flex-shrink-0">
                ...
                <div className="flex items-center gap-2 mb-3 px-1">
                  ...
                  {HUMAN_GATE_COLS.includes(col.id) && (
                    <span className="ml-auto text-xs text-slate-500" title="Human checkpoint">👤</span>
                  )}
        ...
        {/* Live feed sidebar */}
        <aside className="w-72 flex-shrink-0 border-l p-4 flex flex-col gap-3 overflow-hidden"
          style={{ background: '#1A2F45', borderColor: '#243B55' }}>
          <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Live Feed</div>
