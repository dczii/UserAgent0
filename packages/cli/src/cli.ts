...
program
  ...
  .action(async (opts) => {
    ...
    try {
      const { startServer } = await import('@agents-kit/server');
      startServer(port);
...
program
  ...
  .action(async () => {
    ...
    const { DBClient, DB_PATH } = await import('@agents-kit/core');
...
program
  ...
  .action(async (keyValue?: string) => {
    const { readGlobalConfig, setConfigKey } = await import('@agents-kit/core');
