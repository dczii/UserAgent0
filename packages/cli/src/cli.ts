...
program
  ...
  .action(async (opts) => {
    ...
    try {
      const { startServer } = await import('@useragent0/server');
      startServer(port);
...
program
  ...
  .action(async () => {
    ...
    const { DBClient, DB_PATH } = await import('@useragent0/core');
...
program
  ...
  .action(async (keyValue?: string) => {
    const { readGlobalConfig, setConfigKey } = await import('@useragent0/core');
